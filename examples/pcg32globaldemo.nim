...
# In this version of the code, we'll use the global rng, rather than a local one.
...
if nondeterministicSeed:
  ...
  pcg32SRandom(entropy.seed, entropy.seq)
else:
  # Seed with a fixed constant.
  pcg32SRandom(42u64, 54u64)
...
for round in 1 .. rounds:
  ...
  for i in 0 ..< 33:
    add(text, $(pcg32BoundedRand(6u32) + 1))
  ...
  for i in countdown(CARDS, 2):
    let
      chosen = int(pcg32BoundedRand(uint32(i)))
      card = cards[chosen]
